...
**JPG 适用于呈现色彩丰富的图片**，在我们日常开发中，<u>JPG 图片经常作为大的背景图、轮播图或 Banner 图</u>出现。
...
考虑到 PNG 在处理线条和颜色对比度方面的优势，我们主要用它来<u>呈现小的 Logo、颜色简单且对比强烈的图片或背景</u>等。
...
![](https://user-gold-cdn.xitu.io/2018/9/15/165dd6aa9ffcf484?w=872&h=126&f=png&s=32866)
...
此外，还有另一个维护性更强、更加灵活的方案——把判断工作交给后端，由服务器根据 HTTP 请求头部的 Accept 字段来决定返回什么格式的



图片。当 Accept 字段包含 image/webp 时，就返回 WebP 格式的图片，否则返回原图。这种做法的好处是，当浏览器对 WebP 格式图片的兼容支持发生改变时，我们也不用再去更新自己的兼容判定代码，只需要服务端像往常一样对 Accept 字段进行检查即可。
