...
namespace ChapterFunction {
    /函数定义/
    // 1. 声明式
    function fn<T>(name: T): T { return name }
    // 2. 表达式
    const fn1 = function () { }
    // 3. 箭头函数
    const fn2 = () => { };
    ...
    // 完整的函数类型
    const f4: (x: number, y: number) => void = function (x: number, y: number): number { return x + y }

    // 只要参数类型是匹配的，那么就认为它是有效的函数类型，而不在乎参数名是否正确。
    const f5: (a: number, b: number) => number = function (m: number, n: number): number { return m + n }
    ...
    const f7: (props: string) => string = function (props) { return props };
    ...
    f12 = function (name, last: number, ...list) { return name + " " + list.join(" ") }
