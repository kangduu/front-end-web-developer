namespace OrgTreeV1 {
    ...
    class OrgTree {
        ...
        constructor(selector: string, data: IData, cfg: TreeConfig) {
            // tools
        ...
        // 缩放
        enableZoom(delta: number) {
            // 历史记录
            const originTransformStr: string = this.svg.style('transform')
            // 新的值
        ...
        onWheel() {
            ...
            // 滚轮
            wrapper.on('wheel', lodash.throttle((event: any) => {
        ...
        // 计算canvas大小
        calculateCanvasSize(): [number, number] {
            // 浏览器像素比
            const pixelRatio = window.devicePixelRatio || 1;
            // 获取svg的viewBox，从而获取实际大小
            const [minX, minY, width, height] = this.getSvgViewBox()
            const W = parseInt(String(width * pixelRatio)),
                H = parseInt(String(height * pixelRatio));
            return [W, H]
        }
        ...
        generateBase64Image(
            ...
            option: { background?: string, watermark?: string, rotation?: number } = {}
        ) {
            const _this = this;
            // 复制svg
            const dupSvg: any = (document.getElementById(OrgTree.SVG_UID) as Node).cloneNode(true)
            // 重置transform
            ...
            img.onload = function () {
                ...
                if (marker) {
                    ...
                    let textWidth = context.measureText(marker).width,
                        textHegith = 200,
                        pk = 2,
                ...
                if (handler instanceof Function) handler(_exportPath)
        ...
        // 输出为图片
        exportingImage() {
            function downLoad(url: any, fileName: string) {
