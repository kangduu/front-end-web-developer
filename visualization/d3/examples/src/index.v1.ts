(function (window: any) {
    ...
    // mock data
    const Mock: any = window.Mock;
    const mockData = {
        "name": Mock.Random.name(),
        "children": (new Array(20) as any).fill(1).map(() => {
            return Mock.mock({
                "name": Mock.Random.name(),
                "children": (new Array(Math.ceil(Math.random() * 50)) as any).fill(1).map(() => {
                    return Mock.mock({
                        "name": Mock.Random.name(),
                        "children": []
                    })
    ...
    const size: Size = { width: 5000, height: 900 }
    ...
    // 布局
    const treeLayout: Function = D3.tree()
    ...
    const lines = g.selectAll('.links')
        ...
        .attr('stroke', '#313131')
        .attr('stroke-width', 2)
        .attr('d', (d: any) => {//通过三次贝塞尔曲线设置连线的弯曲程度。M：move to，即到控制点 C后设置两个控制点及终点
            return `
                M${d.x},${d.y}
